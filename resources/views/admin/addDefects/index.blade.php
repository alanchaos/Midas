                ...
                <tbody>
                    @foreach($addDefects as $key => $addDefect)
                        <tr data-entry-id="{{ $addDefect->id }}">
                            ...
                            <td>
                                {{ $addDefect->category->defect_cateogry ?? '' }}
                            </td>
                            <td>
                                {{ $addDefect->defect ?? '' }}
                            </td>
                            ...
                            <td>
                                @can('add_defect_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.add-defects.show', $addDefect->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('add_defect_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.add-defects.edit', $addDefect->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('add_defect_delete')
                                    <form action="{{ route('admin.add-defects.destroy', $addDefect->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
