                ...
                <tbody>
                    @foreach($eventControls as $key => $eventControl)
                        <tr data-entry-id="{{ $eventControl->id }}">
                            ...
                            <td>
                                @foreach($eventControl->audience_groups as $key => $item)
                            ...
                            <td>
                                @can('event_control_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.event-controls.show', $eventControl->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('event_control_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.event-controls.edit', $eventControl->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('event_control_delete')
                                    <form action="{{ route('admin.event-controls.destroy', $eventControl->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
