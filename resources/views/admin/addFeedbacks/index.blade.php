                        ...
                        <tr data-entry-id="{{ $addFeedback->id }}">
                            ...
                            <td>
                                @can('add_feedback_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.add-feedbacks.show', $addFeedback->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('add_feedback_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.add-feedbacks.edit', $addFeedback->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('add_feedback_delete')
                                    <form action="{{ route('admin.add-feedbacks.destroy', $addFeedback->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
