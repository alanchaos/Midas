                        ...
                        <tr data-entry-id="{{ $formCategory->id }}">
                            ...
                            <td>
                                @can('form_category_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.form-categories.show', $formCategory->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('form_category_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.form-categories.edit', $formCategory->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('form_category_delete')
                                    <form action="{{ route('admin.form-categories.destroy', $formCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
